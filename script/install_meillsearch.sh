...
get_latest() {
    ...
    if [ -z "$GITHUB_PAT" ]; then
        curl -x 192.168.102.225:7890 -s "$latest_release" > "$temp_file" || return 1
    else
        curl -x 192.168.102.225:7890 -H "Authorization: token $GITHUB_PAT" -s "$latest_release" > "$temp_file" || return 1
    fi
...
download_binary() {
    ...
    curl -x 192.168.102.225:7890 --fail -OL "$GITHUB_REL/$latest/$release_file"
    if [ $? -ne 0 ]; then
        fetch_release_failure_usage
        exit 1
    fi
